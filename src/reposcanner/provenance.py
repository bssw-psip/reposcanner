...
class ReposcannerLabNotebook(AbstractLabNotebook):
    ...
    def __init__(self, notebookOutputDirectory: Union[str, pathlib.Path]) -> None:
        ...
        if not os.path.isdir(
                self._notebookOutputDirectory) or not os.path.exists(
                self._notebookOutputDirectory):
            raise IOError(
                "The notebook output directory {notebookOutputDirectory} either does not exist or \
        ...
        if not os.access(self._notebookOutputDirectory, os.W_OK):
            raise IOError(
                "Reposcanner does not have permissions to write to the notebook output directory \
    ...
    def onTaskCreation(self, task: ManagerTask) -> None:
        ...
        if isinstance(task, ManagerRepositoryRoutineTask):
            ...
            task = self._document.activity(
                "rs:task{taskid}".format(
                    taskid=id(task)), other_attributes=(
                    ('rs:requestType', task.getRequestClassName()), ))
        ...
        if isinstance(request, AnalysisRequestModel):
            ...
            for entity in filesToBeUsedInAnalysis:
                ...
                if isinstance(entity, dataEntities.ReposcannerDataEntity):
                    ...
                    entityID = 'rs:dataentity:nonstandard:{objID}'.format(
                        objID=id(entity))
    ...
    def onTaskCompletion(
        ...
        self._document.wasEndedBy(
            activity=taskID, trigger=agentID, time=endTime, other_attributes=(
                ("rs:wasSuccessful", taskWasSuccessful), ("rs:message", str(taskMessage))))
        ...
        if taskWasSuccessful:
            ...
            for attachment in attachments:
                if isinstance(attachment, dataEntities.ReposcannerDataEntity):
                    ...
                    """
                    dataEntityID = 'rs:dataentity:nonstandard:{objID}'.format(
                        objID=id(attachment))
                    dataEntity = self._document.entity(dataEntityID, (
    ...
    def publishNotebook(self) -> None:
        ...
        dotRepresentation.write_png(
            "{outputPath}/run_{executionID}.png".format(
                outputPath=self._notebookOutputDirectory,
                executionID=executionID))
        dotRepresentation.write_raw(
            "{outputPath}/run_{executionID}.dot".format(
                outputPath=self._notebookOutputDirectory,
                executionID=executionID))
        ...
        with open("{outputPath}/report_{executionID}.md".format(outputPath=self._notebookOutputDirectory, executionID=executionID), 'w') as markdownFile:
            ...
            markdownFile.write(
                "1. Reposcanner (version {versionInfo}, execution id {executionID})\n\n".format(
            ...
            markdownFile.write(
                "----------------------- | ------------------- | -----------\n")
            markdownFile.write(
                "{today} | {versionInfo} | {routinesAndAnalyses}\n\n".format(
                    today=todayDate,
                    versionInfo=versionInfo,
                    routinesAndAnalyses=";".join(routinesAndAnalyses)))
