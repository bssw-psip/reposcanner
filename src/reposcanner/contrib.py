    ...
    def offlineImplementation(self, request: BaseRequestModel, session: Session) -> ResponseModel:
        ...
        if output.fileExists():
            ...
            return responseFactory.createSuccessResponse(
                message="[CommitInfoMiningRoutine] File already exists, \
        ...
        output.setColumnNames(["commitHash",
                               "commitTime",
                               "authorEmail",
                               "authorName",
                               "authorTime",
...
class OnlineCommitAuthorshipRoutine(OnlineRepositoryRoutine):
    ...
    def gitlabImplementation(self, request: BaseRequestModel, session: Session) -> ResponseModel:
        ...
        # Instead, we can get the list of users and their associated emails, and
        # map author/committer identities to Gitlab user accounts.
...
class GambitCommitAuthorshipInferenceAnalysis(DataAnalysis):
    ...
    def execute(self, request: BaseRequestModel) -> ResponseModel:
        ...
        commitLogEntities = [
            entity for entity in data if entity.getCreator() == "CommitInfoMiningRoutine"]

        factory = DataEntityFactory()
        output = factory.createAnnotatedCSVData(
            "{outputDirectory}/gambitAuthorIdentities.csv".format(
                outputDirectory=request.getOutputDirectory()))
        ...
        output.setCreator(self.__class__.__name__)
        output.setDateCreated(datetime.date.today())
        output.setColumnNames(["alias_name",
                               "alias_email",
                               "name",
                               "email",
                               "first_name",
        ...
        contributorFrame = pd.DataFrame(
            contributorNamesAndEmails, columns=[
        ...
        output.writeToFile()
        return responseFactory.createSuccessResponse(
            message="GambitCommitAuthorshipInferenceAnalysis \
...
class OfflineCommitCountsRoutine(OfflineRepositoryRoutine):
    ...
    def offlineImplementation(self, request: BaseRequestModel, session: Session) -> ResponseModel:
        ...
        factory = DataEntityFactory()
        output = factory.createAnnotatedCSVData(
            "{outputDirectory}/{repoName}_offlineCommitCounts.csv".format(
                outputDirectory=request.getOutputDirectory(),
                repoName=request.getRepositoryLocation().getRepositoryName()))
...
class TeamSizeAndDistributionAnalysis(DataAnalysis):
    ...
    def execute(self, request: BaseRequestModel) -> ResponseModel:
        ...
        data = request.getData()
        contributorListEntities = [
            entity for entity in data if entity.getCreator() == "ContributorAccountListRoutine"]
        ...
        analysisCSVOutput = dataEntityFactory.createAnnotatedCSVData(
            "{outputDirectory}/TeamSizeAndDistributionAnalysis_results.csv".format(
                outputDirectory=request.getOutputDirectory()))

        analysisCSVOutput.setReposcannerExecutionID(
            ReposcannerRunInformant().getReposcannerExecutionID())
        analysisCSVOutput.setCreator(self.__class__.__name__)
        analysisCSVOutput.setDateCreated(datetime.date.today())
        analysisCSVOutput.setColumnNames(["URL",
                                          "numberOfContributors",
                                          "numberOfKnownECPContributors",
                                          "numberOfInstitutionsInvolved"])
        ...
        for contributorListEntity in contributorListEntities:
            ...
            for index, individualContributor in contributorListFrame.iterrows():
                if individualContributor["Login Name"] in loginData["login"].values:
                    folksID = loginData.loc[loginData['login'] ==
                                            individualContributor["Login Name"]]['FID'].values[0]
                    ...
                    if folksID in memberData['FID'].values:
                        ...
                        if firstInstitutionOfECPMember is not None and not np.isnan(
                                firstInstitutionOfECPMember):
                            institutionIDsInvolved.add(firstInstitutionOfECPMember)
                        if secondInstitutionOfECPMember is not None and not np.isnan(
                                secondInstitutionOfECPMember):
                            institutionIDsInvolved.add(secondInstitutionOfECPMember)

            analysisCSVOutput.addRecord([repositoryURL,
                                         numberOfContributors,
                                         numberOfKnownECPContributors,
                                         len(institutionIDsInvolved)])

        analysisCSVOutput.writeToFile()

        return responseFactory.createSuccessResponse(
            message="Completed!", attachments=analysisCSVOutput)
