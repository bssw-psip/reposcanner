...
class GitEntityFactory:
    def createRepositoryLocation(
            ...
            expectedHostType=None,
            expectedOwner=None,
            expectedRepositoryName=None):
...
class GitHubAPISessionCreator(VCSAPISessionCreator):
    ...
    def connect(self, repositoryLocation, credentials):
        ...
        if credentials.hasUsernameAndPasswordAvailable():
            ...
            session = pygithub.Github(
                credentials.getToken(), retry=retryHandler)
        else:
            raise RuntimeError(
...
class BitbucketAPISessionCreator(VCSAPISessionCreator):
    ...
    def connect(self, repositoryLocation, credentials):
        if credentials.hasTokenAvailable():
            ...
            credentials.hasUsernameAndPasswordAvailable()
            try:
                bb = Cloud(
                ...
                raise RuntimeError("Bitbucket cloud api needs username and password")
                # this is going to assume the workspace name is the same as username.
                # Will change this if needed
        repository = bb.workspace.get(
            credentials.getUsername()).repositories.get(
            repositoryLocation.getCanonicalName())
        return repository
