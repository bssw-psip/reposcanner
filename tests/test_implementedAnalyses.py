...
def test_TeamSizeAndDistributionAnalysis_analysisReturnsFailureResponseIfLoginDataNotAvailable():
    ...
    # Store has contributor data, but no github_login.csv.
    requestWithNoLoginData.fetchDataFromStore(store)
...
def test_TeamSizeAndDistributionAnalysis_canHandleTestData(tmpdir):
    def generateContributorAccountListFile(store):
        ...
        contributorAccountFile.addRecord(
            ["ajones", "Alice Jones", "alicejones@sandia.gov"])
        # Mark is a non-ECP third-party contributor.
        contributorAccountFile.addRecord(
            ["mmson", "Mark Markson", "markmark@gmail.com"])
        contributorAccountFile.addRecord(
    ...
    def generateECPMembersFile(store):
        ...
        # Alice is both a Sandian and a professor at a university.
        membersFile.addRecord([11, 3320, 2, 10, 8, None])
    ...
    print(scikitEntry)
    # The data came from the scikit repository.
    assert(scikitEntry[0] == 'https://www.github.com/scikit/scikit')
    ...
    # The three contributors are representative of four different institutions
    # (one contributor has two institutional affiliations).
    assert(scikitEntry[3] == 4)
